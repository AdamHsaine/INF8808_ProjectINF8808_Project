...
function processDataForEvolutionAnalysis (crimeData, aggregationType) {
  ...
  /**
   * Obtient la clé temporelle basée sur le type d'agrégation (année, trimestre, mois).
   *
   * @param {string | Date} date La date à partir de laquelle la clé temporelle doit être générée. Cela peut être une chaîne de caractères ou un objet Date.
   * @returns {string} La clé temporelle sous forme de chaîne de caractères, formatée selon l'agrégation demandée.
   *   - Si `aggregationType` est `'year'`, la clé est l'année sous forme de `YYYY`.
   *   - Si `aggregationType` est `'quarter'`, la clé est sous la forme `YYYY-Qn` (où `n` est le trimestre).
   *   - Si `aggregationType` est `'month'`, la clé est sous la forme `YYYY-MM`.
   */
...
function createLineChart (data, highlightType) {
  ...
  /**
   * Met en évidence la ligne et les points associés à une catégorie dans un graphique.
   *
   * Cette fonction réduit l'opacité de toutes les lignes et points, puis met en évidence
   * la ligne et les points associés à la catégorie spécifiée en ajustant leur opacité et
   * en augmentant la largeur du trait pour la ligne et la taille pour les points.
   *
   * @param {string} category Le nom de la catégorie dont la ligne et les points doivent être mis en évidence.
   *        Le nom de la catégorie peut contenir des espaces, qui sont remplacés par des tirets dans les sélecteurs CSS.
   */
  ...
  /**
   * Réinitialise la mise en évidence des lignes et des points dans un graphique.
   *
   * Cette fonction rétablit l'opacité et la largeur des traits des lignes à leurs valeurs par défaut,
   * et rétablit également la taille des points à leur valeur initiale.
   */
