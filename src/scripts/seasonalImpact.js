...
function updateSeasonalAnalysis (crimeData) {
  ...
  const viewType = document.getElementById('seasonal-view-type').value
  const categoryFilter = document.getElementById('seasonal-category').value
  const yearFilter = document.getElementById('seasonal-year').value
  ...
  seasons.forEach(season => {
    ...
    <div class="season-content">
      ...
      <div class="season-trend">
        ${season.key === data.seasonalTrends[season.key].totalCrimes === Math.max(...Object.values(data.seasonalData))
                ? '☝️ Saison la plus active'
                : season.key === data.seasonalTrends[season.key].totalCrimes === Math.min(...Object.values(data.seasonalData))
